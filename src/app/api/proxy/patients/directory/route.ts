...
// GET /patients/directory — Enriched patient directory with filters
export async function GET(req: NextRequest) {
    try {
        const { searchParams } = new URL(req.url);
        const url = new URL(`${API_BASE_URL}/api/v1/patients/directory`);
        ...
        const forwardParams = [
            'directory_scope', 'department_id', 'unit_id', 'provider_team_id',
            'status', 'room', 'bed', 'medical_record_number', 'search',
            'page_id', 'page_size',
        ];
